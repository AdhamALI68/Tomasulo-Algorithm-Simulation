            ...
            if (Ins.IssueCycGet() == -1)
            {
                ...
                for (; RSid < RS.size(); RSid++)
                {
                    if (!RS[RSid].Notbusy() && busylist[Ins.OpGet()] > 0)
                    {
                        RScheck = true;
                        IssureCyc = Cyc;
                        bool errorr; // I can not issue any instruction until all the units are not busy
                        // can not issue the 2 same instructions in the same cycle
                        do
                        {
                            errorr = false;
                            for (const auto& instIssued : Insts)
                            {
                                ...
                                {
                                    errorr = true;
                                    IssureCyc++; // Increment to avoid conflict
                                    break;
                                }
                            }
                        } while (errorr);

                        Ins.IssueCycSet(IssureCyc); // Set resolved issue cycle
                        break;
                    }
                }
                if (!RScheck)
        ...
        if (!rob.IsCountZero())
        {
            ...
            if (entry.busyGet() && Cyc > Ins.WriteCycGet())
            {
                ...
                else
                {
                    ...
                    RegStatus.AssignAvalivblty(entry.DestGet(), false);
                }
...
//    vector<string> InstFromFile = readInstsFromFile("test1.txt");
//    vector<Inst> Insts;
//    int Addr = 0;
